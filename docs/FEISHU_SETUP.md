...
- `DEV_AUTH=false` 后，所有 `/api/*` 请求自动识别飞书用户。

建议上线前补成以下流程：

```text
飞书打开网页
-> 前端通过飞书 JS SDK 或网页应用授权流程获取 auth code
