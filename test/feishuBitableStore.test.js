...
test('converts writes by Feishu field metadata', async () => {
  const calls = [];
  const client = {
    listBitableFields: async (_appToken, tableId) => {
      if (tableId !== 'tbl_reservations') return [];
      return [
        { field_name: '申请时长', type: 1 },
        { field_name: '归还请求次数', type: 1 },
        { field_name: '开始时间', type: 5 },
        { field_name: '计划结束时间', type: 5 },
    ...
    reservations: [
      {
        ...
        purpose: '',
        startedAt: '2026-04-16T02:00:00.000Z',
        plannedEndAt: '2026-04-16T04:00:00.000Z',
        returnedAt: null,
        status: 'reserved',
        returnRequestCount: 0,
        createdAt: '2026-04-16T01:00:00.000Z',
  ...
  assert.equal(calls[0].fields.申请时长, '2');
  assert.equal(calls[0].fields.归还请求次数, '0');
  assert.equal(typeof calls[0].fields.开始时间, 'number');
  assert.equal(typeof calls[0].fields.计划结束时间, 'number');
});
